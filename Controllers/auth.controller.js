...
let signup = asyncHandler(async (req, res) => {
  ...
  res
    .status(201)
    .json({ message: "user created succsefully", payload: newUser });
});
...
let signin = asyncHandler(async (req, res) => {
  ...
  if (founduser.provider === "google") {
    return res.status(400).json({
      message:
        "This account was registered using Google login. Please login with Google.",
    });
  }
  ...
  if (!founduser.isConfirmed) {
    return res
      .status(401)
      .json({ message: "You cannot log in without verifying your email" });
  }
